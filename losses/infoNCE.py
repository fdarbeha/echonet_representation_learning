...
class InfoNCE(nn.Module):
    def __init__(self, args, gar_hidden, genc_hidden):
        ...
        self.args = args
        self.gar_hidden = gar_hidden #128
        self.genc_hidden = genc_hidden# 512
        self.negative_samples = args.negative_samples #32
    ...
    def get(self, x, z, c):
        ...
        if self.args.subsample:
            ...
            """
            if c.size(1) > self.subsample_win: # B x 128
    ...
    def infonce_loss(self, Wc, z, full_z):
        ...
        """
        seq_len = z.size(1) # B x L x 512
        # print('seq len: ', seq_len)
        total_loss = 0
        accuracies = torch.zeros(self.args.prediction_step, 1)
        true_labels = torch.zeros((seq_len * self.args.batch_size,)).long().cuda()
        ...
        for k in range(1, self.args.prediction_step + 1):
            ...
            # print('zk shape: ', z_k.shape)
            # print('wck shape:', Wc_k.shape)

            pos_samples = self.get_pos_sample_f(Wc_k, z_k)
            # print('pos shape', pos_samples.shape)

            neg_samples = self.get_neg_samples_f(Wc_k, z_k, z_neg, k)
            # print('neg shape', neg_samples.shape)
